...
export const getHistorySearchResults = () => async (dispatch, getState) => {
  ...
  try {
    const response = await axios.get(
      `https://aviatick-backend-git-development-aviaticks-projects.vercel.app/api/v1/bookings/booking-history?search=${searchTerm}`,
      {
        headers: {
          accept: "application/json",
          Authorization: `Bearer ${token}`,
...
export const getHistoryByDate = () => async (dispatch, getState) => {
  ...
  try {
    const response = await axios.get(
      `https://aviatick-backend-git-development-aviaticks-projects.vercel.app/api/v1/bookings/booking-history?date=${date}`,
      {
        headers: {
          accept: "application/json",
          Authorization: `Bearer ${token}`,
